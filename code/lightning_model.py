...
class LightningClassicGNN(pl.LightningModule):
    ...
    def __init__(self, classification = True, num_hidden_features=256, dropout_p=0.0, **kwargs: Any) -> None:
        super(LightningClassicGNN,self).__init__()
        
        if "num_node_features" in kwargs.keys():
            self.num_node_features = kwargs["num_node_features"]
        else:
            raise Exception("num_node_features not defined")
        
        if "num_edge_features" in kwargs.keys():
        ...
        if  "num_classes" in kwargs.keys():
            ...
            raise Exception("num_classes not defined") 
